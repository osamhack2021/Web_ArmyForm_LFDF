...
class Survey extends Component {
  ...
  render() {
    ...
    const availableWidth =
      (this.state.window_width > 1820 ? 1820 : this.state.window_width) - 180;
    let column_count = Math.floor(availableWidth / CARD_WIDTH);
    if (column_count <= 0) column_count = 1;

    let page1_data = data1.slice(
      this.state.page1_pointer * column_count,
      (this.state.page1_pointer + 1) * column_count
    );
    let page2_data = data2.slice(
      this.state.page2_pointer * column_count,
      (this.state.page2_pointer + 1) * column_count
    );
    let page3_data = data3.slice(
      this.state.page3_pointer * column_count,
      (this.state.page3_pointer + 1) * column_count
    );
      ...
      <>
        ...
        <div className="background_green">
          <div className="wide_column_container">
            ...
            <Slider
              current={this.state.page1_pointer}
              length={this.state.page1_count}
            />
          </div>
        </div>
        ...
        <div>
          <div className="wide_column_container">
            ...
            <Slider
              current={this.state.page2_pointer}
              length={this.state.page2_count}
            />
          </div>
        </div>
        ...
        <div className="background_green">
          <div className="wide_column_container">
            ...
            <Slider
              current={this.state.page3_pointer}
              length={this.state.page3_count}
            />
          </div>
        </div>
